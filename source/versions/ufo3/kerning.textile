...
The kerning pair members are glyph names or group names. Group names used as the first member of a kerning pair must begin with the @firstKerningGroupPrefix@. Group names used as the second member of a kerning pair must begin with @secondKerningGroupPrefix@. These prefixes are defined in "fontinfo.plist":fontinfo.html#kerning-group-prefixes. Glyphs or groups in the pairs are not required to be in the font.
...
h3(#writingdirection). Writing Direction
...
h3(#kerningpairtypes). Kerning Pair Types
...
h4(#kerningpairexceptions). Exceptions
