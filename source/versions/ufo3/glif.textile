...
| color            | string | The color that should be applied to the image. The format follows the "color definition":#color standard. This attribute is optional. If no color is provided, the image should be drawn using the colors stored in the image data. <span class="editorsnote">This needs to address color setting at the layerinfo.plist level.</span> | None |
