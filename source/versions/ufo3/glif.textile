...
| "link":#unicode        | May occur any number of times. |
...
| fileName         | string      | The image file name. <span class="inlineeditorsnote">This should include the extension, right??</span> | None |
...
All images, regardless of whether they are stored in the _images_ directory or not, must be in PNG format.
...
Need example of image from outside of the UFO.
</div>
...
curve point preceded by two offcurve points, thus creating a bezier curve:
...
h3(#linkref). linkref: A reference to a particular point in the glyph.

<div class="discussionnote">
pointref might be a better name.
</div>
