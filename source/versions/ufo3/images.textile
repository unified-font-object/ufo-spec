...
Image files to be included in the UFO must be stored in the _images_ directory. These images must be in PNG format. Each file must have a unique name but there are no file name requirements %(inlineeditorsnote)(Perhaps we should establish a maximum length and a set of usable characters in the "conventions":conventions.html so that cross platform is as stable as we can make it)%. Duplicate images should be avoided and unreferenced images should not be stored, but these are recommendations not requirements.
