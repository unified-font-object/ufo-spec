...
In UFO 1 and UFO 2, the implication was that if a member of a kerning pair had the same name as a group and a glyph, that member was the group. In UFO 3, this interpretation can be avoided by referencing the _firstKerningGroupPrefix_ and _secondKerningGroupPrefix_ found in "fontinfo.plist":fontinfo.html#kerning-group-prefixes. Knowing the kerning group prefixes allows an application to step through the kerning pair members, perform a string match on the beginning of a member and confidently know if the member is a group or glyph reference. An algorithm for converting UFO 1 and UFO 2 group kerning is detailed below.
